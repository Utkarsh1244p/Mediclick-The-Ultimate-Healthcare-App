...
import com.example.mediclick.NV1
import java.io.File
import java.io.FileOutputStream
import java.io.IOException


open class ResultActivity : BMI() {
    ...
    // Utility function to save a bitmap to external storage
    fun saveBitmap(context: Context, bitmap: Bitmap): Uri? {
        val fileName = "${System.currentTimeMillis()}.jpg"
        val directory = File(Environment.getExternalStorageDirectory(), "MediClick")
        ...
        try {
            ...
            outputStream.flush()
            outputStream.close()
            context.displayToast("Image saved successfully!")
            return Uri.fromFile(file)
        } catch (e: IOException) {
            e.printStackTrace()
            context.displayToast("Failed to save image!")
        }
